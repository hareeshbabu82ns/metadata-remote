  ...
  window.MetadataRemote.Files.Manager = {
    ...
    setupFileControls() {
      ...
      if (filterBtn && filterContainer && filterInput) {
        filterBtn.addEventListener("click", () => {
          ...
          document
            .getElementById("files-sort-dropdown")
      ...
      if (sortBtn && sortDropdown) {
        ...
        sortDirection.addEventListener("click", (e) => {
          e.stopPropagation();
          State.filesSort.direction =
            State.filesSort.direction === "asc" ? "desc" : "asc";
          this.updateSortUI();
          this.renderFileList();
        });
      ...
      document.addEventListener("click", (e) => {
        if (
          !e.target.closest("#files-sort-dropdown") &&
          !e.target.closest("#files-sort-btn")
        ) {
          sortDropdown.classList.remove("active");
    ...
    updateSortUI() {
      ...
      sortIndicator.textContent =
        State.filesSort.direction === "asc" ? "▲" : "▼";

      // Update active option in dropdown
      sortDropdown.querySelectorAll(".sort-option").forEach((option) => {
        option.classList.toggle(
          "active",
          option.dataset.sort === State.filesSort.method,
        );
      });
    },
    ...
    renderFileList() {
      ...
      sortedFiles.forEach((file) => {
        ...
        if (
          State.filesSort.method === "date" ||
          State.filesSort.method === "size"
        ) {
          const metaDiv = document.createElement("div");
          metaDiv.className = "file-meta";
          metaDiv.style.cssText =
            "font-size: 0.75rem; color: #999; margin-top: 0.1rem;";
        ...
        playButton.innerHTML =
          '<span class="play-icon">▶</span><span class="pause-icon">❚❚</span><span class="play-spinner"></span>';
    ...
    async loadFile(filepath, listItem) {
      ...
      document.getElementById("metadata-loading-indicator").style.display =
        "flex";
      ...
      try {
        ...
        if (
          window.MetadataRemote.Metadata &&
        ...
        if (formatLimitations.hasLimitedMetadata) {
          UIUtils.showStatus(
            `Note: ${format.toUpperCase()} files have limited metadata support`,
            "warning",
          );
          ...
          if (data.supportedFields) {
            ...
            allFields.forEach((field) => {
              ...
              const controls = document.querySelector(
                `.apply-field-controls[data-field="${field}"]`,
              );
        ...
        // Load folder field suggestions
        if (window.MetadataRemote.Metadata.FolderFields) {
          window.MetadataRemote.Metadata.FolderFields.loadFolderFields(
            filepath,
            data,
          );
        }
        ...
        if (window.MetadataRemote.Metadata.Clipboard) {
          window.MetadataRemote.Metadata.Clipboard.updateClipboardIndicator();
        }
