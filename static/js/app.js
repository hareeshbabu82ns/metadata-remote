...
const AudioMetadataEditor = {
  ...
  initializeModules() {
    ...
    if (window.MetadataRemote.Metadata.Clipboard) {
      window.MetadataRemote.Metadata.Clipboard.init({
        loadHistory: () => HistoryManager.loadHistory(),
        showButtonStatus: ButtonStatus.showButtonStatus,
        loadFile: this.loadFile.bind(this),
      });
    }

    // Initialize folder fields module
    if (window.MetadataRemote.Metadata.FolderFields) {
      window.MetadataRemote.Metadata.FolderFields.init();
    }

    // UI components
    PaneResize.initializePaneResize();
    PaneResize.initializeHistoryPanelResize(() =>
...
function applyFieldToFolder(field) {
  MetadataEditor.applyFieldToFolder(
    field,
    ButtonStatus.showButtonStatus,
...
function saveFieldToFile(field) {
  ...
  console.log(
    "[app.js saveFieldToFile] Field length:",
    field ? field.length : "null",
  );
