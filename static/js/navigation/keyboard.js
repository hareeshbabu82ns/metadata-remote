  ...
  window.MetadataRemote.Navigation.Keyboard = {
    ...
    setupKeyboardNavigation() {
      ...
      document.addEventListener("click", (e) => {
        ...
        const currentlyEditingField = document.querySelector(
          '.metadata input[type="text"][data-editing="true"]',
        );
        ...
        if (
          ...
          e.target !== currentlyEditingField &&
          mousedownTarget !== currentlyEditingField
        ) {
        ...
        if (e.target.classList.contains("oversized-field-button")) {
          const editingFields = document.querySelectorAll(
            '.metadata input[type="text"][data-editing="true"]',
          );
        ...
        if (
          ...
          document
            .querySelectorAll('.metadata input[type="text"]')
            .forEach((input) => {
              if (input !== e.target) {
                input.blur();
          ...
          if (
            ...
            setTimeout(() => {
              window.MetadataRemote.Metadata.Inference.showInferenceSuggestions(
                e.target.id,
              );
            }, 0);
          }
      ...
      document.addEventListener("keydown", (e) => {
        ...
        if (State.focusedPane === "metadata") {
          ...
          if (
            (e.target.tagName === "BUTTON" ||
              e.target.id === "current-filename" ||
              e.target.classList.contains("extended-fields-toggle") ||
              e.target.classList.contains("new-field-header")) &&
            (e.key === "ArrowUp" ||
              e.key === "ArrowDown" ||
              e.key === "ArrowLeft" ||
              e.key === "ArrowRight")
            ...
            if (
              e.target.classList.contains("inline-choice-file") ||
              e.target.classList.contains("inline-choice-folder")
            ) {
          ...
          if (e.target.tagName === "INPUT" && e.target.type === "text") {
            ...
            if (e.key === "Enter") {
              ...
              const hasSuggestions =
                suggestionsEl && suggestionsEl.classList.contains("active");

              // Hide inference suggestions if they are active
              if (hasSuggestions && window.MetadataRemote.Metadata.Inference) {
                window.MetadataRemote.Metadata.Inference.hideInferenceSuggestions(
                  fieldId,
                );
              }
            ...
            if (
              StateMachine.getState() === StateMachine.States.INLINE_EDIT &&
              State.editingFolder
            ) {
        ...
        if (e.ctrlKey && e.shiftKey && e.key === "S") {
          ...
          const dirBtn = document.getElementById(
            `${State.focusedPane}-sort-direction`,
          );
      ...
      document.addEventListener("keyup", (e) => {
        ...
        if (State.keyHeldDown === e.key) {
          ...
          document
            ...
            .classList.remove("keyboard-navigating");
          document
            .querySelector(".files")
            .classList.remove("keyboard-navigating");
        }
    ...
    registerKeyboardRoutes() {
      ...
      Router.register({ key: "/", state: "*" }, (event, context) => {
      ...
      Router.register(
        ...
        (event) => {
          ...
          if (field && event.target.tagName === "INPUT") {
            ...
            if (fieldId) {
              window.MetadataRemote.Metadata.Editor.triggerFieldDeletion(
                fieldId,
              );
            }
          }
      ...
      // Clipboard shortcuts - Ctrl/Cmd+C to copy metadata
      Router.register(
        { key: "c", state: "*", modifiers: { ctrl: true } },
        (event, context) => {
          // Only handle if in metadata pane and not in an input field
          if (
            State.focusedPane === "metadata" &&
            event.target.tagName !== "INPUT" &&
            event.target.tagName !== "TEXTAREA"
            ...
            event.preventDefault();
            if (window.MetadataRemote.Metadata.Clipboard) {
              window.MetadataRemote.Metadata.Clipboard.copyFromCurrentFile();
            }
          }
          // Otherwise, allow default copy behavior for text selection
        },
      );

      // Clipboard shortcuts - Ctrl/Cmd+V to paste metadata
      Router.register(
        { key: "v", state: "*", modifiers: { ctrl: true } },
        (event, context) => {
          // Only handle if in metadata pane and not in an input field
          if (
            State.focusedPane === "metadata" &&
            ...
            if (
              ...
              window.MetadataRemote.Metadata.Clipboard.hasData()
            ) {
              window.MetadataRemote.Metadata.Clipboard.openPasteModal();
            }
          }
          // Otherwise, allow default paste behavior for inputs
        },
      );

      // Clipboard shortcuts - Ctrl/Cmd+Shift+V to paste to folder
      Router.register(
        { key: "v", state: "*", modifiers: { ctrl: true, shift: true } },
        (event, context) => {
          // Only handle if in metadata pane
          if (
            State.focusedPane === "metadata" &&
            event.target.tagName !== "INPUT" &&
            event.target.tagName !== "TEXTAREA"
            ...
            if (
              ...
              window.MetadataRemote.Metadata.Clipboard.openPasteModal();
              // Auto-select paste to folder after modal opens
              setTimeout(() => {
                const folderBtn = document.getElementById(
                  "paste-to-folder-btn",
                );
        ...
        { priority: 90 }, // Higher priority than regular paste
      );
    ...
    navigateMetadata(key) {
      ...
      if (
        key === "ArrowUp" &&
        activeElement.id === "current-filename" &&
      ...
      if (dynamicFieldsContainer) {
        ...
        const dynamicFields = dynamicFieldsContainer.querySelectorAll(
          'input[data-dynamic="true"], button.oversized-field-button',
        );
      ...
      for (let i = standardFieldIds.length - 1; i >= 0; i--) {
        ...
        const fieldIndex = focusableElements.findIndex(
          (el) => el.id === fieldId,
        );

        if (fieldIndex !== -1) {
          const applyControls = document.querySelector(
            `.apply-field-controls[data-field="${fieldId}"].visible`,
          );
          if (applyControls) {
            const fileBtn = applyControls.querySelector(".apply-file-btn");
            const folderBtn = applyControls.querySelector(
      ...
      const groupedApplyItems = metadataSection.querySelectorAll(
        ".grouped-apply-item",
      );

      const visibleGroupedItems = Array.from(groupedApplyItems).filter(
        (item) => {
      ...
      if (dynamicFieldsContainer) {
        const dynamicFields =
          dynamicFieldsContainer.querySelectorAll(".dynamic-field");
        dynamicFields.forEach((fieldGroup) => {
          const input = fieldGroup.querySelector('input[data-dynamic="true"]');
          if (input) {
            const inputIndex = focusableElements.findIndex(
              (el) => el === input,
            );
            if (inputIndex !== -1) {
              const applyControls = fieldGroup.querySelector(
      ...
      const finalButtonCount = focusableElements.filter(
        (el) =>
    ...
    async saveFilenameInPlace(filenameElement) {
      ...
      try {
        ...
        if (result.status === "success") {
          ...
          if (
            window.AudioMetadataEditor &&
            window.AudioMetadataEditor.loadFiles
          ) {
            window.AudioMetadataEditor.loadFiles(State.currentPath);
          }
          if (
            window.MetadataRemote.History &&
    ...
    handleHeaderNavigation(key) {
          ...
          if (sortBtn && sortDropdown) {
            ...
            setTimeout(() => {
              if (sortDropdown.classList.contains("active")) {
                ...
                const sortOptions =
                  sortDropdown.querySelectorAll(".sort-option");
                if (sortOptions.length > 0) {
                  // Find the currently active option or default to first
                  let activeOption =
                    sortDropdown.querySelector(".sort-option.active") ||
                    sortOptions[0];
      ...
      if (key === "ArrowUp") {
        ...
        if (iconType === "help" && State.previousFocusBeforeHelp) {
          const { pane: prevPane, iconType: prevIconType } =
            State.previousFocusBeforeHelp;
    ...
    returnToPaneFromHeader(pane) {
      ...
      if (pane === "folders") {
        ...
        const fileItems = Array.from(
          document.querySelectorAll('#file-list li:not([aria-hidden="true"])'),
    ...
    updateDynamicFields(fieldIds) {
      ...
      if (
        !window.KeyboardRouter.contexts.form &&
